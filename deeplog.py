    ...
    def train(self, data, epochs=100, criterion=nn.CrossEntropyLoss(), verbose=False):
        ...
        for epoch in range(epochs):
            ...
            for i, (X, y) in enumerate(data):
                ...
                X = X.detach().view(-1, self.window_size, input_size).to(self.device)
...
if __name__ == "__main__":
    ...
    # Parse arguments
    from help_format import CustomHelpFormatter
    ...
    parse_group_deeplog.add_argument('-b', '--batch-size', type=int, default=128, help="batch size for training          (default=   128)")
    parse_group_deeplog.add_argument('-d', '--hidden-dim', type=int, default= 64, help="dimension of hidden layer        (default=    64)")
    parse_group_deeplog.add_argument('-e', '--epochs'    , type=int, default= 10, help="number of epochs to train with   (default=    10)")
    parse_group_deeplog.add_argument('-l', '--layers'    , type=int, default=  2, help="number of layers used by DeepLog (default=     2)")
    parse_group_deeplog.add_argument('-o', '--output'    , type=int, default=  0, help="required output shape            (default='auto')")
    parse_group_deeplog.add_argument('-w', '--window'    , type=int, default= 10, help="window size used by DeepLog      (default=    10)")
